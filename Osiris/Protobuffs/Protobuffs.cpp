...
static std::string runProfileChanger(void* pubDest, uint32_t* pcubMsgSize)
{
	...
	commendation.replace(Field(_pci::cmd_friendly, TYPE_UINT32, (int64_t)ProfileChanger::profileChangerConfig.friendly));
	commendation.replace(Field(_pci::cmd_teaching, TYPE_UINT32, (int64_t)ProfileChanger::profileChangerConfig.teach));
	commendation.replace(Field(_pci::cmd_leader, TYPE_UINT32, (int64_t)ProfileChanger::profileChangerConfig.leader));
	...
	ranking.replace(Field(_pri::rank_id, TYPE_UINT32, (int64_t)ProfileChanger::profileChangerConfig.rank));
	ranking.replace(Field(_pri::wins, TYPE_UINT32, (int64_t)ProfileChanger::profileChangerConfig.wins));
	msg.replace(Field(_gc2ch::ranking, TYPE_STRING, ranking.serialize()));
	msg.replace(Field(_gc2ch::player_level, TYPE_INT32, (int64_t)ProfileChanger::profileChangerConfig.level));
	msg.replace(Field(_gc2ch::player_cur_xp, TYPE_INT32, (int64_t)ProfileChanger::profileChangerConfig.exp));

	if (ProfileChanger::profileChangerConfig.ban_type != 0 && ProfileChanger::profileChangerConfig.ban_time != 0)
	{
		msg.replace(Field(_gc2ch::penalty_reason, TYPE_INT32, (int64_t)ProfileChanger::profileChangerConfig.ban_type));
		msg.replace(Field(_gc2ch::penalty_seconds, TYPE_INT32, (int64_t)ProfileChanger::profileChangerConfig.ban_time));
	}
...
{
	if (messageType == k_EMsgGCCStrike15_v2_MatchmakingGC2ClientHello && ProfileChanger::profileChangerConfig.enabled)
	{
		auto packet = runProfileChanger(pubDest, pcubMsgSize);
...
{
	ProfileChanger::g_Inventory->Presend(unMsgType, pubData, cubData);

	if (unMsgType == k_EMsgGCAdjustItemEquippedState)
	{
		return ProfileChanger::g_Inventory->Presend(unMsgType, pubData, cubData);
	}
