...
void Config::load(size_t id) noexcept
{
    ...
    for (size_t i = 0; i < aimbot.size(); i++) {
		...
		if (aimbotJson.isMember("Standalone RCS")) aimbotConfig.standaloneRecoilControl = aimbotJson["Standalone RCS"].asBool();
...
void Config::save(size_t id) const noexcept
{
    ...
    for (size_t i = 0; i < aimbot.size(); i++) {
		...
		aimbotJson["Standalone RCS"] = aimbotConfig.standaloneRecoilControl;
