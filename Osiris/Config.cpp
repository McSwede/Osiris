...
static void from_json(const json& j, Config::Aimbot& a)
{
    ...
    read_number(j, "Recoil control X", a.recoilControlX);
    read_number(j, "Recoil control Y", a.recoilControlY);
    read<value_t::boolean>(j, "Standalone RCS", a.standaloneRecoilControl);
    read_number(j, "Standalone RCS Ignore Shots", a.shotsFired);
}
...
static void from_json(const json& j, Config::Misc& m)
{
    ...
    read<value_t::boolean>(j, "Human Bunny hop", m.humanBunnyHop);
    read_number(j, "Bhop Hit Chance", m.bhop_hit_chance);
    read_number(j, "Bhop Restricted Limit", m.hops_restricted_limit);
    read_number(j, "Max Hops Hit", m.max_hops_hit);
    ...
    read<value_t::boolean>(j, "Left Knife", m.leftKnife);
